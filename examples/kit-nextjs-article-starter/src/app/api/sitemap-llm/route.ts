...
function shouldIncludeUrl(url: string): boolean {
  try {
    ...
    if (urlPath.toLowerCase().includes('landing-page')) return false;
  ...
  try {
    ...
    if (urls.length === 0) {
      ...
      urls = [
        ...
        { loc: `${baseUrl}/Article-Page`, lastmod: today, priority: '0.8' },
        { loc: `${baseUrl}/Articles`, lastmod: today, priority: '0.8' },
      ];
    }
